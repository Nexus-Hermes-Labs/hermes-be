...
#[async_trait]
pub trait GuildRoleRepository: Repository<GuildRole, Uuid, Error = RepositoryError> + Send + Sync {
    /// Get all roles for a guild (ordered by position desc)
    async fn find_by_guild(
        &self,
        guild_id: Uuid,
    ) -> Result<Vec<GuildRole>, Self::Error>;
    ...
    async fn find_default_role(
        &self,
        guild_id: Uuid,
    ) -> Result<Option<GuildRole>, Self::Error>;
