...
pub trait AuthSessionRepository: Repository<AuthSession, Uuid, Error = RepositoryError> {
    ...
    /// Find all active (non-revoked, non-expired) sessions for a user_profile
    async fn find_active_by_user_id(&self, user_id: Uuid) -> Result<Vec<AuthSession>, Self::Error>;

    /// Find all sessions (including revoked/expired) for a user_profile
    ...
    /// Revoke all sessions for a user_profile (logout from all devices)
