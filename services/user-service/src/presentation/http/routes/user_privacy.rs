...
pub fn user_privacy_routes() -> Router<AppState> {
    Router::new()
        .route("/:user_id/privacy", get(user_privacy::get_privacy_settings))
        .route("/:user_id/privacy/dm", put(user_privacy::update_dm_privacy))
        .route(
        ...
        .route("/:user_id/privacy/preset", post(user_privacy::apply_preset))
}
