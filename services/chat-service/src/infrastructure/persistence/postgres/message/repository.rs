...
/// Column list for the shared `messages` table, selecting only channel rows.
...
/// `PostgreSQL` implementation of `MessageRepository` (channel messages only).
#[derive(Debug)]
...
impl Repository<Message, Uuid> for PostgresMessageRepository {
    ...
    async fn find_all(&self) -> Result<Vec<Message>, Self::Error> {
        let sql = format!(
            "SELECT {MSG_COLS} FROM messages WHERE channel_id IS NOT NULL AND is_deleted = FALSE ORDER BY created_at DESC"
        );
    ...
    async fn delete(&self, id: Uuid) -> Result<(), Self::Error> {
        sqlx::query(
            "UPDATE messages SET is_deleted = TRUE, updated_at = NOW() WHERE id = $1 AND channel_id IS NOT NULL",
        )
        .bind(id)
        .execute(&self.pool)
        .await?;
    ...
    async fn exists(&self, id: Uuid) -> Result<bool, Self::Error> {
        let exists: bool = sqlx::query_scalar(
            "SELECT EXISTS(SELECT 1 FROM messages WHERE id = $1 AND channel_id IS NOT NULL AND is_deleted = FALSE)",
        )
        .bind(id)
        .fetch_one(&self.pool)
        .await?;
    ...
    async fn count(&self) -> Result<i64, Self::Error> {
        let count: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM messages WHERE channel_id IS NOT NULL AND is_deleted = FALSE",
        )
        .fetch_one(&self.pool)
        .await?;
        Ok(count)
...
impl MessageRepository for PostgresMessageRepository {
    ...
    async fn find_by_id_and_channel(
        ...
        let sql = format!(
            "SELECT {MSG_COLS} FROM messages WHERE id = $1 AND channel_id = $2 AND is_deleted = FALSE"
        );
        let row = sqlx::query_as::<_, MessageRow>(&sql)
            .bind(id)
            .bind(channel_id)
